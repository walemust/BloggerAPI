...
const filterAndSort = (req, res, next) => {
    ...
    req.findFilter = {}
    if (author) req.findFilter.owner = new RegExp(`${author}`, 'pi')
    if (title) req.findFilter.title = new RegExp(`${title}`, 'pi')
    if (tags) {
        const tagAttributes = tags.split(',').map((tag) => new RegExp(`${tag}`, 'pi'))
